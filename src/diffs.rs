...
impl<'a> Diff for PyDiff<'a> {
    ...
    fn add_line(&mut self, old_line: u64, new_line: u64, line: &[u8]) {
        self.lines.push(PyTuple::new(self.py,
                                     &[old_line.into_object(self.py),
                                       new_line.into_object(self.py),
                                       PyBytes::new(self.py, line).into_object(self.py)]).into_object(self.py));
    }
